...
class NeuralNetwork(object): # 三层神经网络，采用批量更新算法
    ...
    def batch_bp(self, x, t):
        while True:
            for k in range(len(t)): # 所有样本
                self.__calc(x[k])
            ...
            # 判断是否满足终止条件
            if len(self.record_e) > 1 and abs(self.record_e[len(self.record_e) - 1] - self.record_e[len(self.record_e) - 2]) < self.theta:
                print('达到终止条件')
